...
async function loadQuestion(){
  ...
  qLength = data.fruits.length;
  showQuestion(data);
}

function showQuestion(data){
  total.innerHTML = `${qCount} of ${data.fruits.length} Questions`
  score.innerHTML = `Score: ${userScore} / ${data.fruits.length}`
  question.src = data.fruits[qIndex].question;
  options.innerHTML = `${data.fruits[qIndex].options.map((option) => `
...
function selectOption(data){
  ...
  for(let i = 0; i<answers.length; i++){
     answers[i].onclick = function(){
      ...
      let correctAnswer = data.fruits[qIndex].answer;
