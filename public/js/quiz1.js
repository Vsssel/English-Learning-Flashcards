...
async function loadQuestion(){
  ...
  qLength = data.school.length;
  showQuestion(data);
}

function showQuestion(data){
  total.innerHTML = `${qCount} of ${data.school.length} Questions`
  score.innerHTML = `Score: ${userScore} / ${data.school.length}`
  question.src = data.school[qIndex].question;
  options.innerHTML = `${data.school[qIndex].options.map((option) => `
...
function selectOption(data){
  ...
  for(let i = 0; i<answers.length; i++){
     answers[i].onclick = function(){
      ...
      let correctAnswer = data.school[qIndex].answer;
